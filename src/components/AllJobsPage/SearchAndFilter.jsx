...
const SearchAndFilter = () => {
    ...
    useEffect(() => {
        const baseUrl = "https://job-hai-client.vercel.app/api/v1/jobs?page=1&limit=5";
        let url = baseUrl;
        const queryParams = {};
            ...
            <form action="" className="form">
                <div className="filter">
                    ...
                    <div className="type-row  ">
                        <span className="text h-full">Types</span>
                        <select
                            className="type-select"
                            onChange={(e) => setTypeFilter(e.target.value)}
                            value={typeFilter}
                        >
                            <option value="" className="p-4">default</option>
                            {Job_Type?.map((type, i) => {
                                return (
...
const Wrapper = styled.div`
    background-color: rgb(168, 168, 157);
    padding: 1.2rem 1rem;
    display: flex;
    align-items: center;
    border-radius: 6px;
